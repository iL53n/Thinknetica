...
class Railway
  ...
  def menu_trains
    loop do
      ...
      when 5 then take_place_or_capacity
      when 6 then set_route_to_train
      when 7 then forward_train
      when 8 then back_train
      when 0 then break
      else puts INVALID_INDEX
      end
    end
  ...
  def trains_list
    puts '*** Список поездов ***'
    return puts('Список поездов пуст!') if @trains.empty?
  ...
  def add_carriage_to_train
    ...
    if train.is_a?(PassengerTrain)
      ...
      input = gets.to_i
      carriage = PassengerCarriage.new(input)
    else
      print 'Введите объем: '
      input = gets.to_i
      carriage = CargoCarriage.new(input)
    end
  ...
  def menu_trains_view
    ...
    puts '5 - занять место или объем в вагоне'
    puts '6 - назначить маршрут поезду'
    puts '7 - переместить поезд вперед по маршруту'
    puts '8 - переместить поезд назад по маршруту'
    puts '0 - главное меню'
    print 'Ваш выбор: '
  end
